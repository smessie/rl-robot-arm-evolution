...
class QTable:
    def __init__(self, n_states: int, n_actions: int, alpha: float, gamma: float) -> None:
        ...
        self.table = dict() # state -> [0, 1.2, 0, 2.]

    def update(self, state: np.ndarray, new_state: np.ndarray, action_index: int, reward: float) -> None:
        state = tuple(state)
        ...
        self.table[state][action_index] = (1 - self.alpha) * old_value + self.alpha * (
