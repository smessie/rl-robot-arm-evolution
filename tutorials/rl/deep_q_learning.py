...
class DeepQLearner():
    ...
    def __init__(self, env_path: str, urdf_path: str, use_graphics: bool = False, network_path: str = "") -> None:
        print("----------------initializing--------------------")
    ...
    def predict(self, state: np.ndarray, stochastic: bool = False) -> int:
        if stochastic and np.random.rand() < self.eps:
            with torch.no_grad():
                ...
                probs, indices = torch.topk(x, self.N_ACTIONS, sorted=True)
